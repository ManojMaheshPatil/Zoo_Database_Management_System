...
-- Find out the number of classes of animals in the database
SELECT CLASS, COUNT(ANIMALID)
FROM ANIMAL
GROUP BY CLASS;
-- Order the number of employees and customers based on their states
...
CREATE TRIGGER total_revenue
BEFORE INSERT
ON exhibit 
FOR EACH ROW
SET NEW.TotalAmount=NEW.NoOfHours*NEW.FeePerHour;

INSERT INTO exhibit(ExhibitID,Name,Location,Hours,NoOfHours,FeePerHour) values(100,"New Zoo","North Zone","7:00-10:00",3,50);

START TRANSACTION;
insert into PERSON values ('MANOJ PATIL', 'MALLESWARAM', 'BANGALORE', '560003', 'KARNATAKA', '1950/05/02');
SAVEPOINT transaction1;

insert into PERSON values ('ABC', 'MG ROAD', 'CHENNAI', '660003', 'TAMIL NADU', '1971/11/13');
